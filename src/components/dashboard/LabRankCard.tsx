          ...
          {labs.top.map((l) => (
            <Link key={`${l.professor}|${l.lab ?? ''}`} href={`/projects?q=${encodeURIComponent(l.professor)}`} className="dash-list-row">
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ display: 'block', width: 70, height: 4, background: 'var(--chart-track)', borderRadius: 2 }}>
                  <span style={{ display: 'block', width: `${(l.count / max) * 100}%`, height: '100%', background: 'var(--chart-1)', borderRadius: 2 }} />
                </span>
