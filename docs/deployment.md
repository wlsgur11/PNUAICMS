...
> **이전 완료(2026-06~07).** 지금은 이 문서대로 교내 자체 서버 PC 에서 운영 중이고 DB 도 그 서버 안에 있습니다. 실제 운영 절차·환경변수·장애 대응은 `docs/notion/20-배포와-운영.md` 를 보세요. 이 문서는 구성 배경과 도구 선택 근거로 남겨 둡니다.

자체 서버 PC 로 옮기기 위해 쓴 문서입니다. 앱은 `output: 'standalone'` + Docker 이미지로 패키징되어, 배포 도구(Compose/Swarm/K8s)가 무엇이든 같은 이미지를 씁니다.
